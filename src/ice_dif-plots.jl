...
function calc_and_plot_diffH_grnd(path2files, file_names, xvals, yvals, time_index, layout2plot, path2save, plot2save)
    ...
    levels2D = -4500:100:4500
    dif_array[d_array[:, 2, :, :].<=0] .= Inf

    figure_size = (750 * layout2plot[2], 700 * layout2plot[1])
    plot_name_2D = path2save * "diffH_grnd_" * plot2save * ".png"
    plot_multivar(dif_array, xvals, yvals, "H_grnd change (m)", levels2D, clrmp=:RdBu, fgsz=figure_size, ptsv=plot_name_2D, cont=c_array, cont_lvls=[0, 1])
end
...
function calc_and_plot_diffuxy_s(path2files, file_names, xvals, yvals, time_index, layout2plot, path2save, plot2save)
    ...
    levels2D = -7000:100:7000
    dif_array[c_array[:, :, :].<=0] .= Inf

    figure_size = (750 * layout2plot[2], 700 * layout2plot[1])
    plot_name_2D = path2save * "diffuxy_s_" * plot2save * ".png"
    plot_multivar(dif_array, xvals, yvals, "uxy_s change (m/a)", levels2D, clrmp=:vikO, fgsz=figure_size, ptsv=plot_name_2D, cont=c_array, cont_lvls=[0, 1])
end